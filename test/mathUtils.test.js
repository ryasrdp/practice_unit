...
describe('mathUtils', () => {
  describe('add()', () => {
    ...
    it('should suffer from floating-point precision (bug)', () => {
      assert.notStrictEqual(mathUtils.add(0.1, 0.2), 0.3);
      assert.approximately(mathUtils.add(0.1, 0.2), 0.3, 1e-15);
    });
    it('should concatenate when given strings (bug, no type check)', () => {
      assert.strictEqual(mathUtils.add('2', 3), '23');
      assert.strictEqual(mathUtils.add(2, '3'), '23');
      assert.strictEqual(mathUtils.add('2', '3'), '23');
    });
    it('should return Infinity on overflow (bug, no range check)', () => {
  ...
  describe('subtract()', () => {
    ...
    it('should coerce numeric strings to numbers (bug, no type check)', () => {
      assert.strictEqual(mathUtils.subtract('5', 3), 2);
      assert.strictEqual(mathUtils.subtract(5, '3'), 2);
      assert.strictEqual(mathUtils.subtract('5', '3'), 2);
    });
  ...
  describe('multiply()', () => {
    ...
    it('should return 0 on underflow (bug, no range check)', () => {
